...
namespace uStora.Service
{
    public interface IProductService : ICrudService<Product>, IGetDataService<Product>
    {
        ...
        IEnumerable<Product> GetAllPaging(int page, int brandid, int manuId, string sort, int pageSize, out int totalRow);
    ...
    public class ProductService : IProductService
    {
        ...
        public IEnumerable<Product> GetAllPaging(int page, int brandid, int manuId, string sort, int pageSize, out int totalRow)
        {
            ...
            if (manuId != 0)
            {
                query = query.Where(x => x.ManufactorId == manuId).OrderBy(x => x.Price);
            }
        ...
        public bool CanPurchase(long productId, int quantity)
        {
